...
type CardDecision = Pick<Decision, "id" | "action_taken" | "reasoning" | "created_at"> & { order_number?: string | null };

// One agent decision: verdict, what it was about, when, and the reasoning.
// On an order's own page the order is already known, so `title` can say something else.
export function DecisionCard({ decision, title }: { decision: CardDecision; title?: string }) {
