...
- `/orders/<id>`: one order: customer, total, statuses, Shopify's fraud check (its risk level and advice, what raised the risk, whether the billing and shipping addresses match, and when it was checked), the order in Shopify ("In Shopify", read live when the page opens: each shipment's status, holds and what's left to ship, with Mark as fulfilled (tracking number, carrier, link, email the customer), Put on hold (reason, note) and Release hold, each confirmed in a short form and offered only when Shopify allows it, plus a log of what was done from here, by the seller or the agent), what was in it (items, quantities, how many are still to ship, prices), every agent decision about it with the reasoning and its rating, and "Open in Shopify". "Back to orders" returns to the filtered list it came from
- `/decisions`: the agent's decisions, newest first, as cards (order, verdict, headline, reasoning as bullets). The order links to its page, and a HOLD or FULFILL card links straight to acting on it there ("Put on hold in Shopify", "Mark fulfilled in Shopify"). Each card asks "Right call?" with thumbs up (Yes) and down (No), saved on click; clicking the pressed one again clears it. A No opens an optional note ("What should it have done?"). Above the cards: the share rated right over every decision so far, per verdict too, and filters for All, Not rated and Marked wrong (`?show=unrated`, `?show=wrong`). A card rated under a filter stays in view until the page is reloaded. Skipped runs can't be rated
